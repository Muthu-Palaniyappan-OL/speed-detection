...
FPS = 25
KM_PER_PIXEL = 25
MAX_SKIPPABLE_FRAME = 10

############################################################

car_colors = [(255, 80, 0),(255, 105, 0),(255, 155, 0),(255, 200, 0)]
        ...
        if(not len(classes) == 0):
            for classId, confidence, box in zip(classes.flatten(), confidences.flatten(), boxes):
                if (within_box(box[0], box[1], BOX2) or within_box(box[0], box[1], BOX1)):
                    ...
                    ids.append(obj_id)
                    cv2.rectangle(frame, box, car_colors[current_car_color_index], thickness=2)
                    cv2.circle(frame, (box[0] + int(box[2]/2), box[1]+ int(box[3]/2)), 4, (0, 0, 255), -1)
                    vehicle_centers[obj_id][3] = 0
                    cv2.rectangle(frame, (box[0], box[1]-20, 80, 20), car_colors[current_car_color_index], thickness=-1)
                    cv2.putText(frame, str(obj_id) + ' [' + str(speed//KM_PER_PIXEL) + ']', (box[0]+2, box[1]-5), cv2.FONT_HERSHEY_PLAIN, 1,(255, 255, 255), 1)
            ...
            for i in vehicle_list:
                if i not in ids:
                    ...
                    if vehicle_centers[i][3] >= MAX_SKIPPABLE_FRAME:
